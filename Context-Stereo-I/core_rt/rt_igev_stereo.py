...
class IGEVStereo(nn.Module):
    def __init__(self, args):
        ...
        self.cost_agg = hourglass_fusion(8)
    ...
    def forward(self, image1, image2, iters=4, flow_init=None, test_mode=False):
        ...
        with autocast(enabled=self.args.mixed_precision):
            ...
            volume = build_gwc_volume(match_left, match_right, self.args.max_disp//4,8)
            #volume = build_norm_correlation_volume(match_left, match_right, self.args.max_disp//4)
            #volume = self.corr_stem(volume)
            geo_encoding_volume = self.cost_agg(volume, features_left)

            # Init disp from geometry encoding volume
            prob = F.softmax(self.classifier(geo_encoding_volume).squeeze(1), dim=1)
            # prob = F.softmax(geo_encoding_volume.squeeze(1), dim=1)
        ...
        for itr in range(iters):
            ...
            with autocast(enabled=self.args.mixed_precision):
                # disp_error = self.align(disp, img1_small, img2_small)
