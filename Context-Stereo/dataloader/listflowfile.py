...
def dataloader(filepath): #'/home/admin1/sceneflow/'
    filepath += '/'
    classes = [d for d in os.listdir(filepath) if os.path.isdir(os.path.join(filepath, d))] #['frames_disparity', 'frames_cleanpass', 'driving_disparity', 'driving__frames_cleanpass', 'monkaa__frames_cleanpass', 'monkaa_disparity']
    image = [img for img in classes if img.find('frames_cleanpass') > -1] #['frames_cleanpass','driving__frames_cleanpass', 'monkaa__frames_cleanpass']
    disp = [dsp for dsp in classes if dsp.find('disparity') > -1] #['frames_disparity', 'driving_disparity',  'monkaa_disparity']
    
    monkaa_path = filepath + [x for x in image if 'monkaa' in x][0] #'/home/admin1/sceneflow//monkaa__frames_cleanpass'
    monkaa_disp = filepath + [x for x in disp if 'monkaa' in x][0] #'/home/admin1/sceneflow//monkaa_disparity'

    monkaa_dir = os.listdir(monkaa_path) #makaa_path里面的文件
